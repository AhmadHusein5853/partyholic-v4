...
if __name__ == '__main__':
    ...
    # add to the app.run(...) the parameter host with your own local ip adress
    # you can find your own local ip using the command (ifconfig -a) on linux : usually starts with 192.168.0.*
    # example:
    # app.run(host="192.168.0.10", threaded=True, port=5000)
    # Hint : in this way the mobile app can't send a request to the server
    # because the server on the local machine is using http and not (https)
    # and android don't let any http traffic
    # to be send !!
    # therefore this server need to be deployed on heroku first ,
    # and Heroku will take care of TLS certificate
